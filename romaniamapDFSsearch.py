...
def buscaEmProfundidade(romania_map, estado_inicial, estado_objetivo):
    ...
    while True:
        ...
        #Remover da borda (LIFO - pop do final)
        node_atual = borda.pop()
        ...
        for transicao in node_atual.state.transitions:
            ...
            if filho_estado.name not in explorados:
                ...
                #Inserir na borda
                borda.append(filho_node)

# --- Execução DFS ---
print("\n--- 📉 Resultado da Busca em Profundidade (DFS) ---")
caminho_dfs, custo_dfs = buscaEmProfundidade(romania_map, s0, s1)
if isinstance(caminho_dfs, str): print(caminho_dfs)
else: print(f"Caminho: {' -> '.join(caminho_dfs)} (Custo: {custo_dfs} km)")