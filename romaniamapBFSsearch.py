...
def buscaEmLargura(romania_map, estado_inicial, estado_objetivo):
    node = Node(state=estado_inicial)

    # se começamos no destino a função ja retorna o estado e o custo=0, nem chama a reconstruirCaminho
    if node.state.name == estado_objetivo.name:
        return [estado_inicial.name], 0
    ...
    while True:
        ...
        for transicao in node_atual.state.transitions:
            ...
            filho_node = Node(
                ...
                pai=node_atual,
                action=transicao,
                distance=node_atual.distance + transicao.distance
            )

            if filho_estado.name not in explorados:
                # Teste de objetivo NO FILHO
                if filho_node.state.name == estado_objetivo.name:
                    return reconstruir_caminho(filho_node)

                # Inserir na borda
                borda.put(filho_node)

# --- Execução BFS ---
print("--- 🌊 Resultado da Busca em Largura (BFS) ---")
caminho_bfs, custo_bfs = buscaEmLargura(romania_map, s0, s1)
if isinstance(caminho_bfs, str): print(caminho_bfs)
else: print(f"Caminho: {' -> '.join(caminho_bfs)} (Custo: {custo_bfs} km)")